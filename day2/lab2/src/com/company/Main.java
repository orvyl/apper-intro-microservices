...
public class Main {
    ...
    public static void main(String[] args) throws BookTooYoungException, InvalidFieldFormatException {
        IdGenerator coolIdGen = new CoolIdGenerator(); //ORACLE
        IdGenerator oldIdGen = new IdGeneratorImpl(); //MYSQL

        BookService bookService = new BookService(coolIdGen);

        bookService.registerBook("1" , "Orvyl", "Orvbyl", 2010);
        bookService.getAllBooks().forEach(new Consumer<Book>() {
            @Override
